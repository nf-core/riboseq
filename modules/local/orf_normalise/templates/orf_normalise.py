#!/usr/bin/env python3
"""Normalise a per-sample ORF caller output to unified BED12 + sidecar TSV.

One template, five callers. ${caller} is interpolated by Nextflow from
meta.caller and selects the per-caller parser, classifier and score mapper.
...
smORF / other. Any ORF with aa_length <= 100 is promoted to smORF.

`SCORE_DIRECTIONS` documents whether a caller's native score is "lower is
better" (min) or "higher is better" (max). The catalogue-merge step uses
this to pick the best per-ORF call; within this module it informs the
BED12 score-to-0-1000 mapping.
"""
import csv
import gzip
import io
...
# `blocks` is a list of (genomic_start_0, genomic_end_excl) tuples in genomic
# order. `bed_score` is the integer 0-1000 BED score. `raw_score` is the
# native caller score (p-value, phase score, Bayes factor) kept for the
# sidecar TSV.
# ----------------------------------------------------------------------------
...
def main():
    if CALLER not in PARSERS:
        sys.exit(f"orf_normalise: unknown meta.caller='{CALLER}'; expected one of {sorted(PARSERS)}")
