...
The canonical backbone constrains the gene set used by the DTE step. Just before `anota2seq` / `deltaTE` the pipeline runs `FILTER_COUNTS_CANONICAL`, which restricts the Salmon gene matrix to gene_ids present in the canonical backbone. Under the default AGAT-derived canonical the gene set equals the full GTF's gene set so this is a no-op. With a restricted user-supplied `--canonical_gtf` (e.g. MANE Select), genes that are not in MANE Select drop out of the DTE comparison. This is intentional - it keeps `gene_id` agreement between the Ribo-seq numerator (canonical-only) and the RNA-seq denominator - but it means non-MANE genes don't appear in the DTE results.tsv. If you need TE for the full Salmon set, supply a non-restricted `--canonical_gtf` (or omit it entirely).
...
**Tier 2: per-ORF DESeq2 interaction model.** The pipeline reuses the same deltaTE DESeq2 model from the gene-level path but at ORF resolution. A pre-processing step joins the per-ORF P-site count matrix (`<outdir>/orf_quantification/orf_psite_counts.tsv`) with the gene-level Salmon RNA-seq matrix via `orf_to_gene.tsv` from the catalogue, producing one combined count table whose rows are ORFs and whose RNA columns hold the host gene's count replicated across all ORFs sharing that gene. The deltaTE module then fits `~ condition + seq_type + condition:seq_type`, treating each ORF row as a feature. Results land under `<outdir>/dte/orf_level/`, including the combined input matrix (`orf_combined_counts.tsv`), per-contrast deltaTE result tables, classified ORF lists, and the standard deltaTE diagnostic plots (PCA, heatmap, fold-change, interaction p-value distribution).
...
- **Low-count ORFs.** uORFs, smORFs and low-abundance novel ORFs frequently have sparse P-site counts. DESeq2 dispersion estimation is unreliable on rows with too many zeros. Pass DESeq2-side options via `--extra_orf_dte_args` (forwarded to the deltaTE module for the ORF-level call); inspect the dispersion plot before interpreting results.
