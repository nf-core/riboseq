...
- **Low-count ORFs.** uORFs, smORFs and low-abundance novel ORFs frequently have sparse P-site counts. DESeq2 dispersion estimation and anota2seq's RVM are both unreliable on rows with too many zeros. Pass method-side options via `--extra_orf_dte_args` (deltaTE) or `--extra_anota2seq_run_args` (anota2seq); inspect the diagnostic plots before interpreting results.
