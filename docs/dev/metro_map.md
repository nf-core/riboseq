...
nf-metro render assets/metro_map.mmd \
  -o docs/images/nf-core-riboseq_metro_map.svg \
  --no-chrome-css --mode light

python -c "import cairosvg; cairosvg.svg2png(
    url='docs/images/nf-core-riboseq_metro_map.svg',
    write_to='docs/images/nf-core-riboseq_metro_map.png', output_width=2265)"
...
nf-metro render assets/metro_map.mmd \
  -o docs/images/nf-core-riboseq_metro_map_animated.svg \
  --animate

# Ensure trailing newlines on SVGs (required by pre-commit)
for f in docs/images/nf-core-riboseq_metro_map.svg \
         docs/images/nf-core-riboseq_metro_map_animated.svg; do
  sed -i '' -e '$a\' "$f"
done
```
...
nf-metro serve assets/metro_map.mmd --open --shutdown-after-complete -- \
    nextflow run nf-core/riboseq -profile test,docker --outdir results

# Or serve from the committed SVG (no source needed)
nf-metro serve docs/images/nf-core-riboseq_metro_map.svg --open --shutdown-after-complete -- \
    nextflow run nf-core/riboseq -profile test,docker --outdir results
```
...
Several stations are only reachable via non-default parameters (e.g.
`--extended_orf_analysis`, `--run_ribotricer`, `--run_rpbp`, `--run_price`,
`--skip_stringtie false`, `--umi_dedup_tool umicollapse`) since the map draws
every optional branch as if it were active. A single default `-profile test`
run will report several of those as drift; that is expected and does not mean
