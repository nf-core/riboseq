...
- [nf-core/riboseq: Output](#nf-coreriboseq-output)
  ...
  - [Preprocessing](#preprocessing)
    ...
    - [fastp](#fastp)
    - [Read length equalisation](#read-length-equalisation)
    - [BBSplit](#bbsplit)
    - [rRNA removal](#rrna-removal)
      - [SortMeRNA (default)](#sortmerna-default)
      - [Bowtie2](#bowtie2)
  ...
  - [Novel transcript discovery (StringTie / hybrid GTF)](#novel-transcript-discovery-stringtie--hybrid-gtf)
  - [Hybrid STAR alignment](#hybrid-star-alignment)
  - [ORF predictions](#orf-predictions)
    ...
    - [Rp-Bp](#rp-bp)
    - [PRICE](#price)
    - [ORF catalogue (cross-sample)](#orf-catalogue-cross-sample)
    - [Per-ORF P-site quantification](#per-orf-p-site-quantification)
  - [P-site identification](#p-site-identification)
    - [riboWaltz](#ribowaltz)
    - [plastid](#plastid)
  - [Quantification](#quantification)
  - [Translational efficiency](#translational-efficiency)
    - [anota2seq outputs](#anota2seq-outputs)
    - [deltaTE outputs](#deltate-outputs)
    - [ORF-level DTE outputs](#orf-level-dte-outputs)
    - [Fold change plots](#fold-change-plots)
    - [Diagnostic plots](#diagnostic-plots)
      - [anota2seq diagnostic plots](#anota2seq-diagnostic-plots)
      - [deltaTE visualizations](#deltate-visualizations)
      - [Interpreting deltaTE diagnostic plots](#interpreting-deltate-diagnostic-plots)
    - [MultiQC](#multiqc)
...
- `preprocessing/read_length_stats/`
  - `*.riboseq_readlength_stats.tsv`: Per-sample Ribo-seq read length statistics, produced by seqkit stats and used to derive the RNA-seq hard-trim target length.
- `preprocessing/read_length_equalisation/`
  - `*_equalised.fq.gz`: If `--save_trimmed` is specified, RNA-seq FastQ files hard-trimmed to match the Ribo-seq read length distribution.

</details>

When `--equalise_read_lengths` is specified, RNA-seq reads are hard-trimmed with Trim Galore! to the same length as the Ribo-seq reads (derived from the Ribo-seq read length statistics, or set explicitly via `--equalise_read_lengths_target`), removing read-length as a confound between the two library types before alignment-based TE quantification.
...
- `other/ribotricer/`
  - `*_pos.wig` and `*_neg.wig`: Forward- and reverse-strand read coverage tracks produced as a side effect of Ribotricer's `detect-orfs` step
  </details>
...
- `psites/ribowaltz/offset_plot/<SAMPLE>/`
  - `*.pdf`: P-site offset plots for each read length
- `psites/ribowaltz/ribowaltz_qc/`
...
- `psites/plastid/tracks/`
  - `*.bedgraph`: Forward- and reverse-strand P-site bedGraph tracks, mapped at each read's P-site offset, for visualisation in a genome browser
...
- `genome/index_te/`
  - `kallisto/`: Directory containing the kallisto index used for `--te_quantification_method pseudo --pseudo_aligner kallisto`.
