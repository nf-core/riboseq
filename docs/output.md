...
The RNA-seq denominator is quantified (Salmon pseudo-alignment) against the full reference transcriptome augmented with the novel intergenic transcripts, so ORFs on novel genes keep a host-gene RNA-seq row and are retained in the analysis. Quantifying against the canonical reference alone would leave every novel-gene ORF without a denominator and silently drop it, so only novel ORFs on known genes would survive. Canonical genes keep their full-isoform counts (the one-transcript-per-gene backbone used for ORF calling would undercount multi-isoform genes). The hybrid RNA-seq gene matrices are published under `quantification/salmon_hybrid/` (same Salmon layout as `quantification/salmon/`, filenames prefixed `salmon.merged.hybrid.`).
