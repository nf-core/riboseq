...
- [#197](https://github.com/nf-core/riboseq/issues/197) - Reuse the TE pseudo-alignment Salmon index for strandedness detection when both are needed with `--pseudo_aligner salmon`, since they were already built from identical inputs and k-mer size. Only affects the `--save_reference` publish layout: the shared index now appears solely under `genome/index_te/`, not duplicated into `genome/index/` ([@pinin4fjords](https://github.com/pinin4fjords))
