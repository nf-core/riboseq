...
- [#168](https://github.com/nf-core/riboseq/issues/168) - ORF-level differential translation. Two paths: (a) gene-level TE numerator re-aggregation that sums only `canonical_cds` ORFs from the catalogue, keeping the gene-level TE clean of uORF / dORF dynamics; (b) per-ORF DTE via the existing anota2seq / deltaTE engines fed by `DTE_COUNTS_PREP` (joins per-ORF Ribo-seq counts with gene-level Salmon RNA-seq counts via `orf_to_gene.tsv`). Gated on `--extended_orf_analysis true` + catalogue exists + `--skip_plastid false`. Adds `--run_dotseq` as a no-op placeholder for future DOTSeq DOU integration. Row-independence caveat: ORFs sharing a gene-level Salmon denominator are perfectly correlated after the join ([@pinin4fjords](https://github.com/pinin4fjords))
