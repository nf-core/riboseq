...
- [#165](https://github.com/nf-core/riboseq/issues/165) - Add `--extended_orf_analysis` (default `false`) which routes the hybrid GTF (#164) into the genome-BAM ORF callers (Ribo-TISH `predict`, Ribotricer `prepare-orfs`) so novel intergenic ORFs are within scope. RiboCode and transcriptome-BAM consumers stay on the canonical backbone pending #171's second STAR pass. When the flag is set without a novel-transcript source, the pipeline warns and falls back to canonical ([@pinin4fjords](https://github.com/pinin4fjords))
...
- [#170](https://github.com/nf-core/riboseq/issues/170) - Add PRICE (Erhard et al. 2018) as an opt-in ORF caller via `--run_price`. PRICE estimates a shared cohort-level codon-position model via EM and is invoked once across the riboseq cohort (`gedi -e IndexGenome` + `gedi -e Price`). Honours `--extended_orf_analysis` by feeding the hybrid GTF when active. Outputs land under `<outdir>/price/` ([@pinin4fjords](https://github.com/pinin4fjords))
...
- [#167](https://github.com/nf-core/riboseq/issues/167) - Cross-sample ORF catalogue. Gathers per-sample, per-caller ORF predictions (Ribo-TISH, RiboCode, Ribotricer, Rp-Bp, PRICE), normalises each to a unified BED12 + sidecar TSV, then merges into a cohort-level catalogue using a class-aware strategy (transcript-ID grouping for annotated multi-exon CDS, 80% reciprocal overlap for single-exon novel intergenic and smORFs). Emits `orf_catalogue.{bed12,tsv}`, `orf_to_gene.tsv`, and an AA FASTA under `<outdir>/orf_catalogue/`, plus a MultiQC custom-content per-class count table. Gates on `--extended_orf_analysis true` and at least one ORF caller enabled. Uses the upstream `orftable_fasta_gtf_buildorfcatalogue` subworkflow ([@pinin4fjords](https://github.com/pinin4fjords))
- [#166](https://github.com/nf-core/riboseq/issues/166) - ORF-level P-site quantification. Expands the cohort BED12 catalogue into codon-start BED6 positions (frame defined by each ORF's own ATG, not GTF `phase`), then runs per-sample `bedtools intersect` against plastid wiggle tracks to assemble an ORF x sample count matrix (`orf_psite_counts.tsv`, zero-filled for ORFs absent from a sample). Gated on `--extended_orf_analysis true` + at-least-one-caller + `--skip_plastid false`. Matrix published under `<outdir>/orf_quantification/` ([@pinin4fjords](https://github.com/pinin4fjords))
- [#168](https://github.com/nf-core/riboseq/issues/168) - ORF-level differential translation. Two paths: (1) gene-level TE numerator re-aggregation that sums ONLY `canonical_cds` ORFs from the catalogue, keeping the gene-level TE clean of uORF/dORF dynamics; (2) per-ORF DTE via the existing anota2seq / deltaTE engines fed by `DTE_COUNTS_PREP` (joins per-ORF Ribo-seq counts with gene-level Salmon RNA-seq counts via `orf_to_gene.tsv`). Gated on `--extended_orf_analysis true` + catalogue exists + `--skip_plastid false`. Adds `--run_dotseq` as a placeholder for future DOTSeq DOU integration (currently a no-op pending nf-core/modules#11742). Row-independence caveat: ORFs sharing a gene-level Salmon denominator are perfectly correlated after the join ([@pinin4fjords](https://github.com/pinin4fjords))
