...
- [#197](https://github.com/nf-core/riboseq/issues/197) - Have `PREPARE_GENOME` build the Salmon index needed for automatic strandedness detection whenever the samplesheet has an `auto`-strandedness sample, instead of an unreachable check (`'salmon' in prepare_tool_indices`, where nothing ever added `'salmon'`) that left index building to run lazily and redundantly inside the preprocessing subworkflow ([@pinin4fjords](https://github.com/pinin4fjords))
- [#197](https://github.com/nf-core/riboseq/issues/197) - Treat TE pseudo-alignment (`--pseudo_aligner salmon`) and strandedness detection as one Salmon index concept instead of two: drop the separate `SALMON_INDEX_TE` process (identical inputs and k-mer size to `SALMON_INDEX`, just a different publish path), and honour a user-supplied `--salmon_index` for TE pseudo-alignment too, which it previously had no effect on. `--save_reference` now publishes the shared index solely under `genome/index/` (previously `genome/index_te/` when only TE pseudo-alignment ran) ([@pinin4fjords](https://github.com/pinin4fjords))
