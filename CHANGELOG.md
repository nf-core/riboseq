...
- [#196](https://github.com/nf-core/riboseq/issues/196) - Streamline the output folder structure: nest PRICE/RiboCode/Rp-Bp under `orf_predictions/`, move ORF-level DTE under `translational_efficiency/orf_level/`, relocate `hybrid_star` to `alignment/hybrid_star` (index to `genome/index/hybrid_star`), gate the PRICE index behind `--save_reference`, and rename `orf_catalogue` to `orf_predictions/catalogue`, `orf_quantification` to `quantification/orf_level`, and `stringtie` to `transcript_assembly/stringtie` ([@pinin4fjords](https://github.com/pinin4fjords))
