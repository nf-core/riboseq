...
- [#197](https://github.com/nf-core/riboseq/issues/197) - Consolidate Salmon indexing in `PREPARE_GENOME`: strandedness auto-detection and TE pseudo-alignment (`--pseudo_aligner salmon`) now share one eagerly-built index, and a user-supplied `--salmon_index` is honoured for both, matching how nf-core/rnaseq treats them upstream. Previously, strandedness detection depended on an unreachable check that left its index built lazily elsewhere, and TE pseudo-alignment built an identical index separately via `SALMON_INDEX_TE` while ignoring `--salmon_index`. `--save_reference` now publishes the shared index under `genome/index/` only (previously also `genome/index_te/`) ([@pinin4fjords](https://github.com/pinin4fjords))
