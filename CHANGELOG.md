...
- [#217](https://github.com/nf-core/riboseq/pull/217) - Patch `gedi/indexgenome` so the GEDI genome index is self-contained. GEDI records absolute paths in the `.oml`/`.fi` it writes, including one to the source FASTA, which was staged at the task work directory root and so never became part of the emitted index. On executors where work directories are not a shared filesystem (AWS Batch with Fusion, and cloud executors generally) that path was never materialised, and `--run_price` failed in `PriceCodonInference` with `Could not read sequence from file`. The FASTA is now copied into the index directory and indexed from there, so the recorded path travels with the index ([@FelixKrueger](https://github.com/FelixKrueger))
