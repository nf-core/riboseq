...
- [#217](https://github.com/nf-core/riboseq/pull/217) - Re-pin `gedi/indexgenome` and `gedi/price` (nf-core/modules#12451) so the GEDI genome index is relocatable and `--run_price` works on cloud executors. GEDI recorded an absolute path to the source FASTA in the `.fi` it wrote, pointing at a staged input outside the emitted index directory that AWS Batch with Fusion never materialised, so PRICE failed in `PriceCodonInference` with `Could not read sequence from file`. `indexgenome` now indexes from a copy inside the index directory, which makes GEDI record a relative name, and `price` rewrites the `.oml`'s absolute member paths onto the staged index ([@pinin4fjords](https://github.com/pinin4fjords))
