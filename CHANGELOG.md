...
- [#168](https://github.com/nf-core/riboseq/issues/168) - Extend `--translational_efficiency_method` to the Tier 2 ORF-level DTE path. anota2seq is now an alternative to deltaTE at ORF resolution via the new `ANOTA2SEQ_ANOTA2SEQRUN_ORF` alias, consuming the same method-agnostic `DTE_COUNTS_PREP` output. ORF-level results land under `<outdir>/dte/orf_level/<method>/`; the deltaTE ORF outputs move from `<outdir>/dte/orf_level/` to `<outdir>/dte/orf_level/deltate/` to make room for the symmetric anota2seq directory ([@pinin4fjords](https://github.com/pinin4fjords))
- [#168](https://github.com/nf-core/riboseq/issues/168) - Add DOTSeq as a third ORF-level option for `--translational_efficiency_method` (alongside anota2seq and deltate). DOTSeq fits both ORF-level differential translation efficiency (DTE, DESeq2 + ashr) and the package's own Differential ORF Usage (DOU, per-gene beta-binomial GLM, no anota2seq/deltate counterpart) from the combined ORF-level count matrix plus the ORF catalogue annotation. The deferred `--run_dotseq` placeholder parameter is removed; selecting `dotseq` requires `--extended_orf_analysis true` and a configured ORF caller. Outputs published under `<outdir>/dte/orf_level/dotseq/` ([@pinin4fjords](https://github.com/pinin4fjords))
