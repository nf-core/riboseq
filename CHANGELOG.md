...
- [#197](https://github.com/nf-core/riboseq/issues/197) - Drop the separate `SALMON_INDEX_TE` process: TE pseudo-alignment (`--pseudo_aligner salmon`) and strandedness detection were already building identical Salmon indexes from the same inputs and k-mer size under two different names. Both now share one `SALMON_INDEX` build, published solely under `genome/index/` with `--save_reference` (previously `genome/index_te/` when only TE pseudo-alignment ran) ([@pinin4fjords](https://github.com/pinin4fjords))
