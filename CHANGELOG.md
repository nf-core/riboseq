...
- [#238](https://github.com/nf-core/riboseq/pull/238) - Drop dead `versions.yml` `saveAs` guards in `conf/modules.config` for modules that now report their version purely via a topic-channel `eval()` tuple and never write the file, and move the workflow's topic-versions collection next to where it's consumed, ahead of MultiQC ([@pinin4fjords](https://github.com/pinin4fjords))
- [#239](https://github.com/nf-core/riboseq/pull/239) - Adopt Nextflow strict-syntax conventions (`channel.` instead of `Channel.`, named closure parameters instead of implicit `it`, drop unused variables/`take:` parameters) across the pipeline's own code, and drop the redundant `/main` suffix from three local-subworkflow `include` paths flagged in #234 review ([@pinin4fjords](https://github.com/pinin4fjords))
