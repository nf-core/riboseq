...
- [#201](https://github.com/nf-core/riboseq/issues/201) - Anchor the subworkflow-level `withName` selectors so `GTF_HYBRIDMERGE_GFFCOMPARE` and `ORFTABLE_FASTA_GTF_BUILDORFCATALOGUE` config applies in-pipeline; the hybrid GTF had been silently dropping every novel transcript ([@pinin4fjords](https://github.com/pinin4fjords))
