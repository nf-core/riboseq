...
13. (optional, when the ORF catalogue is built) ORF-level differential translational efficiency: a DESeq2 interaction-model fit per ORF (`DESEQ2_ORF_DTE`) plus a re-aggregation of canonical-CDS ORF counts into a cleaner gene-level numerator before anota2seq / deltaTE ([#168](https://github.com/nf-core/riboseq/issues/168)).
