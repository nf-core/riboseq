...
13. (optional, when the ORF catalogue is built) ORF-level differential translational efficiency: the user's selected DTE method (`--translational_efficiency_method anota2seq|deltate`) is run at ORF resolution after a join step expands the gene-level RNA-seq matrix onto ORF rows, plus a re-aggregation of canonical-CDS ORF counts into a cleaner gene-level numerator before anota2seq / deltaTE ([#168](https://github.com/nf-core/riboseq/issues/168)).
